...
import { Users, Calendar, Wallet, ChevronRight, ArrowUpRight, ArrowDownRight, Clock, Target } from 'lucide-react'
import Layout from '../components/Layout'
import StatCard from '../components/StatCard'
import RevenueChart from '../components/RevenueChart'
import CircularGauge from '../components/CircularGauge'
import PromoCard from '../components/PromoCard'
import { companies, upcomingMeetings, recentActivity, formatCompactDH, formatDH } from '../data/mockData'
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <PromoCard onOpenChat={() => window.dispatchEvent(new Event('open-chat'))} />
        <StatCard
          label="Clients signés"
          value={main.stats.clientsSignes}
          trend={main.stats.clientsSignesTrend}
          icon={Users}
          sparkline={main.stats.clientsSpark}
        />
        <StatCard
          label="Réunions tenues"
          value={main.stats.reunionsTenues}
          trend={main.stats.reunionsTenuesTrend}
          trendLabel={`${main.stats.reunionsTenuesTrend >= 0 ? '+' : ''}${main.stats.reunionsTenuesTrend}`}
          icon={Calendar}
          sparkline={main.stats.reunionsSpark}
        />
        <StatCard
          label="Chiffre d'affaires"
          value={formatCompactDH(main.stats.chiffreAffaires)}
          trend={main.stats.chiffreAffairesTrend}
          icon={Wallet}
          sparkline={main.revenue.slice(-6).map((d) => d.value)}
        />
      </div>
          ...
          <div className="flex flex-col gap-2">
            {upcomingMeetings.map((meeting) => (
              <div key={meeting.id} className="flex items-start gap-2.5 rounded-lg border border-ink-100 bg-ink-50/60 p-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[12px] font-bold text-ink-800">{meeting.title}</p>
                  <p className="truncate text-[11px] text-ink-500">{meeting.client}</p>
                  <p className="mt-0.5 text-[10.5px] font-bold text-ink-700">{meeting.date}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-2.5 flex flex-col divide-y divide-ink-100">
          {recentActivity.map((activity) => (
            <div key={activity.id} className="flex items-center gap-3 py-2 first:pt-1">
              ...
              />
              <p className="min-w-0 flex-1 truncate text-[12.5px] text-ink-700">{activity.text}</p>
              <span className="shrink-0 text-[11px] text-ink-400">{activity.time}</span>
              {activity.amount && (
                <span
                  className={
                    'shrink-0 text-[11.5px] font-bold ' + (activity.type === 'up' ? 'text-emerald-500' : 'text-rose-500')
                  }
                >
                  {activity.amount}
                </span>
